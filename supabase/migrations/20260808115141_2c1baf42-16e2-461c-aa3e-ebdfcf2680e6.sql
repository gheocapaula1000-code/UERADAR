-- Valori attesi vuoti = non specificati: evitano falsi MISMATCH quando il
-- provider non fornisce il dato.
...
BEGIN
  ...
  IF NOT FOUND THEN
    RETURN jsonb_build_object('ok', false, 'code', 'NO_SUBSCRIPTION_ROW');
  END IF;
  IF _row.provider_customer_id IS NOT NULL AND _exp_customer IS NOT NULL
     AND _row.provider_customer_id IS DISTINCT FROM _exp_customer THEN
    RETURN jsonb_build_object('ok', false, 'code', 'CUSTOMER_MISMATCH');
  END IF;
