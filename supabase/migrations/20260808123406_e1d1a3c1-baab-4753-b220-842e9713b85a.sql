-- Replay-safe: indice unico parziale sulla sessione registrata nell'intento
CREATE UNIQUE INDEX IF NOT EXISTS ueradar_checkout_intents_session_uidx
  ON public.ueradar_checkout_intents (session_id)
  WHERE session_id IS NOT NULL;

-- Nuova firma: il primo binding richiede la Checkout Session esatta.
DROP FUNCTION IF EXISTS public.ueradar_billing_apply_subscription(uuid, text, uuid, timestamptz, text, text, text, jsonb);
...
-- Settle: validità del lease riverificata sotto lock immediatamente prima
-- della chiusura. Un worker scaduto o un evento già chiuso non mutano nulla.
...
BEGIN
  ...
  IF _ev.status <> 'processing'
     ...
     OR _ev.lease_expires_at <= now() THEN
    RETURN jsonb_build_object('ok', false, 'code', 'LEASE_LOST');
  END IF;
